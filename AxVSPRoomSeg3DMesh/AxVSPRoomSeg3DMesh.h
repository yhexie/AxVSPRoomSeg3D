...
typedef bg::model::point<double, 3, bg::cs::cartesian> Boost_Point;
typedef bg::model::box<Boost_Point> Boost_Box;
typedef std::pair<Boost_Box, size_t> Boost_Value;
typedef bgi::rtree<Boost_Value, bgi::quadratic<16> > Boost_RTree;

class AxVSPRoomSeg3DMesh
{
public:
	AxVSPRoomSeg3DMesh();
	~AxVSPRoomSeg3DMesh();
	...
	void getTiangleBoundingBox(Eigen::Vector3d v0, Eigen::Vector3d v1, Eigen::Vector3d v2, 
		Eigen::Vector3d& tmpmin_, Eigen::Vector3d& tmpmax_);
public:
	openvdb::FloatGrid::Ptr grid_occupied;//占用概率
	openvdb::Int32Grid::Ptr grid_dist2sky;//距离天空的距离，而后标记自由空间（室内空间0）
