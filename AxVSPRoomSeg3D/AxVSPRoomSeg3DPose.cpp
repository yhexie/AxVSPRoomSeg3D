...
AxVSPRoomSeg3DPose::AxVSPRoomSeg3DPose():
...
void AxVSPRoomSeg3DPose::doInitialize()
{
...
int AxVSPRoomSeg3DPose::doEDT()
{
	...
	for (int i = startId; i < endId; i++)
	{
		...
		char a[10], b[10];
		sprintf(a, "%d", i);
		//std::string path = "D:\\Experiments\\A03Experiments\\scan";
		//tPath = "C:\\data\\scan";
		std::string filename = tPath + a + ".pcd";
		std::cout << "读取文件：" << filename << std::endl;
		pcl::PointCloud<pcl::PointXYZRGB>::Ptr xyz(new pcl::PointCloud<pcl::PointXYZRGB>);
		if (pcl::io::loadPCDFile<pcl::PointXYZRGB>(filename, *xyz) == -1) // load the file
		{
			PCL_ERROR("文件读取失败！\n");
			break;
		}
		std::cout << "points: " << xyz->points.size() << std::endl;
		Eigen::Vector4f    sensor_position = xyz->sensor_origin_;
		if (sensor_position == Eigen::Vector4f::Zero())
		{
			PCL_ERROR("缺少传感器位置信息！\n");
			break;
		}
		...
		{
			int coord_min_x = (int)std::min(std::floor(coor_min_original.x()) - 2, coor_min.x());
			int coord_min_y = (int)std::min(std::floor(coor_min_original.y()) - 2, coor_min.y());
			int coord_min_z = (int)std::min(std::floor(coor_min_original.z()) - 2, coor_min.z());
			int coord_max_x = (int)std::max(std::ceil(coor_max_original.x()) + 2, coor_max.x());
			int coord_max_y = (int)std::max(std::ceil(coor_max_original.y()) + 2, coor_max.y());
			int coord_max_z = (int)std::max(std::ceil(coor_max_original.z()) + 2, coor_max.z());
			coor_min[0] = coord_min_x;
			coor_min[1] = coord_min_y;
			coor_min[2] = coord_min_z;
			coor_max[0] = coord_max_x;
			coor_max[1] = coord_max_y;
			coor_max[2] = coord_max_z;
		}
...
void AxVSPRoomSeg3DPose::doFillInnerSphere()
{
...
void AxVSPRoomSeg3DPose::doFillInnerSphereEx()
{
	pcl_cloud.reset(new pcl::PointCloud<pcl::PointXYZI>);
...
//随机填充，加快速度
void AxVSPRoomSeg3DPose::doFillInnerSphereRandom()
{
	pcl_cloud.reset(new pcl::PointCloud<pcl::PointXYZI>);
	std::vector<AxDistCell> listDistCells;
	grid_dist_map_threshold = grid_dist_map->deepCopy();//存放阈值分割之后的值
...
void AxVSPRoomSeg3DPose::doBuildGraph()//构造图
{
...
void AxVSPRoomSeg3DPose::doConnectComponent()
{
...
void AxVSPRoomSeg3DPose::doWavefrontGrowing()
{	
	bool finished = false;
...
void AxVSPRoomSeg3DPose::doSaveSpherePlyFile()
{
...
void AxVSPRoomSeg3DPose::saveInitialEdges()
{
	FloatGrid::Accessor grid_dist_map_acc = grid_dist_map->getAccessor();
...
int AxVSPRoomSeg3DPose::doSaveFinnalVDB()
{
	std::string vdbFilename, imgFilename, gridName;
...
void AxVSPRoomSeg3DPose::evalMinMax(EDTGrid::Ptr dist_, dataCell& minVal, dataCell& maxVal)
{
	/// @todo optimize
...
void AxVSPRoomSeg3DPose::setStartAndEndFrame(int start_id, int end_id)
{
	startId = start_id;
	endId = end_id;
}

void AxVSPRoomSeg3DPose::setParameters(float door_width, float overlap_r, float min_seed_vol)
{
...
void AxVSPRoomSeg3DPose::setFilePath(std::string path)
{
	tPath =  path;
}

void AxVSPRoomSeg3DPose::setVoxelSize(float size_vox)
{
	VOX_SIZE = size_vox;
}
